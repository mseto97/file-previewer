...
class IEManager : MMFileImport, MMFileExport {
        ...
        do {
            ...
            for m in jsonData {
                ...
                if fvalidate.validateData(fullpath: m.fullpath, type: m.type) {
                    ...
                    if fvalidate.validateMetadata(type: m.type, checks: checks){
                        ...
                        if m.type == "document" {
                            ...
                            if !Library.files.contains(file) {
                ...
                if !fvalidate.validateData(fullpath: m.fullpath, type: m.type) || !fvalidate.validateMetadata(type: m.type, checks: checks) {
                    ...
                    if (allValid) {
                        IEManager.errorMessages.append(">  Files not successfully imported for \(getFilename(from: filename)):\n\n")
                        allValid = false
                    }
                    
                    if !m.fullpath.isEmpty {
                        IEManager.errorMessages.append("\t\(m.fullpath):\n")
                    } else {
                        IEManager.errorMessages.append("\tThis piece of media did not contain a path -- it was not added to the library.\n")
                    }
                    
                    let followMessages = fvalidate.getErrorMessages(fullpath: m.fullpath, type: m.type, checks: checks)
                    
                    for message in followMessages {
                        IEManager.errorMessages.append("\t\u{25AA} ")
                        IEManager.errorMessages.append(message)
                        IEManager.errorMessages.append("\n")
                    }
                    IEManager.errorMessages.append("\n")
                }
            }
    ...
    func write(filename: String, items: [MMFile]) throws {
        ...
        if (!(getFilename(from: fileLocation) == "library.json")) {
